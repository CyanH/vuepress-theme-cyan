...
export default {
  ...
  methods: {
    reciveResult(txt) {
      ...
      audio.setAttribute(
        "src",
        "https://yating.online/mm/speech?text=" + txt.you
      );
      audio.play();
    },
    loadJs(url, callback) {
  ...
  mounted() {
    ...
    //     }
    // } else {
    this.loadJs("/loveqingzi/live2d/live2d.min.js");
    // this.loadJs("/live2d/live2d.js")
    window.onload = () => {
      let r = Math.floor(Math.random() * 88);
      loadlive2d(
        "live2d",
        "https://cdn.jsdelivr.net/gh/mumudadi/live2dw@latest/assets/live2d-widget-model-Pio/assets/" +
          r +
          ".json"
      );
    };
